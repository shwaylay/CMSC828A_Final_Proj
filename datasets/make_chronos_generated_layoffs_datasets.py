...
for symbol in layoffs['Symbol'].unique():
    ...
    for i, row in temp.reset_index().iterrows():
        ...
        directory = "chronos_stocks_layoffs\\chronos"
        directory2 = "chronos_stocks_layoffs\\real"
        ...
        idx2 = pd.date_range(row['Date_layoffs'] - pd.Timedelta(days=45), row['Date_layoffs'] + pd.Timedelta(days=44), freq='D', inclusive='both')
        ...
        if len(generated) >= target_length:
            ...
            # Find real dataset and copy it
            real_directory = "stocks_layoffs\\"+industry+ f"\\{symbol}{i}.csv"
            if os.path.exists(real_directory):
                print("Saving real data")
                real_df = pd.read_csv(real_directory)
                real_df.to_csv(directory2 + f"\\{symbol}{i}.csv")
            else:
                print("Coulnt find: ", symbol+str(i))
